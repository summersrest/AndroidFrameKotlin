...
import com.sum.framekt.base.dialog.formatter.Formatter

/**
 * @author  LiuJiang
 * Desc:    列表弹窗
 */
class SimpleListDialog<T>(private var context: Context) {
    ...
    private var formatter: Formatter<T>? = null
    ...
    fun setFormatter(formatter: Formatter<T>): SimpleListDialog<T> {
    ...
    fun show() {
        val itemList: List<String> = data.map {
            formatter?.format(it) ?: run { it as String }
        }

        checkedPosition = checkedPosition.takeUnless { checkedPosition >= data.size } ?: -1

        XPopup.Builder(context).maxHeight(maxHeight).maxWidth(maxWidth).dismissOnBackPressed(isDismissOnBackPressed).dismissOnTouchOutside(isDismissOnTouchOutside)
            .isDarkTheme(isDarkTheme).run {
